...
{
	loadModel(path);
	// Calcule les bounding boxes après avoir chargé le modèle
...
void Model::calculateBoundingBoxes() {
	frustumBox = BoundingBox();
	collisionBox = BoundingBox();

	for (const auto& mesh : meshes) {
		for (const auto& vertex : mesh.vertices) {
			frustumBox.update(vertex.position);
...
bool Model::isVisible(const Frustum& frustum, const glm::mat4& modelMatrix) const {
	...
	BoundingBox transformedBox = frustumBox.getTransformed(modelMatrix);
