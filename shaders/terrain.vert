...
void main()
{
    FragPos = vec3(model * vec4(aPos, 1.0)); // Calcule la position dans l'espace monde
    Normal = mat3(transpose(inverse(model))) * aNormal; // Transforme la normale
    TexCoords = aTexCoords; // Transmet les coord. de texture

    gl_Position = projection * view * vec4(FragPos, 1.0); // Position finale
}